...
export function createApiRouter(supabase: SupabaseClient): Router {
  ...
  router.post('/api/auth/login', async (req, res) => {
    ...
    await logActivity(supabase, user.id, user.username, user.role, 'LOGIN', `User ${user.nama_lengkap} berhasil login.`, req);
    res.json({ id: user.id, username: user.username, nama_lengkap: user.nama_lengkap, role: user.role, status: user.status });
  });
  ...
  router.post('/api/students', async (req, res) => {
    ...
    const newStudent = {
      id: 'std-' + uuid(), nis, nama_lengkap, jenis_kelamin, kelas, jurusan, tahun_ajaran, nama_wali, no_hp_wali, alamat,
      status: status || 'aktif', created_at: new Date().toISOString(), updated_at: new Date().toISOString()
    };
    const { data, error } = await supabase.from('students').insert(newStudent).select().single();
    if (error) return res.status(500).json({ message: error.message });
    await logActivity(supabase, user.id, user.username, user.role, 'CREATE_STUDENT', `Menambahkan siswa baru: ${nama_lengkap} (NIS: ${nis})`, req);
  ...
  router.put('/api/students/:id', async (req, res) => {
    ...
    await logActivity(supabase, user.id, user.username, user.role, 'UPDATE_STUDENT', `Mengubah data siswa: ${nama_lengkap} (NIS: ${nis})`, req);
    res.json(data);
  });
  ...
  router.delete('/api/students/:id', async (req, res) => {
    ...
    if (error) return res.status(500).json({ message: error.message });
    await logActivity(supabase, user.id, user.username, user.role, 'DELETE_STUDENT', `Menghapus siswa: ${student?.nama_lengkap} (NIS: ${student?.nis}) beserta seluruh data tagihan/pembayarannya.`, req);
  ...
  router.post('/api/students/import', async (req, res) => {
    ...
    for (const s of students) {
      ...
      const { error } = await supabase.from('students').insert({
        id: 'std-' + uuid(), nis: s.nis.toString(), nama_lengkap: s.nama_lengkap,
        jenis_kelamin: s.jenis_kelamin || 'Laki-laki', kelas: s.kelas || 'X', jurusan: s.jurusan || 'Umum',
        tahun_ajaran: s.tahun_ajaran || '2025/2026', nama_wali: s.nama_wali || '-', no_hp_wali: s.no_hp_wali || '-',
        alamat: s.alamat || '-', status: 'aktif', created_at: new Date().toISOString(), updated_at: new Date().toISOString()
      });
      if (!error) importedCount++; else skippedCount++;
    }
    if (importedCount > 0) {
      await logActivity(supabase, user.id, user.username, user.role, 'IMPORT_STUDENTS', `Mengimport ${importedCount} data siswa baru. (Dilewati: ${skippedCount})`, req);
    }
  ...
  // PAYMENT TYPES CRUD
  // ==========================================
  router.get('/api/payment-types', async (req, res) => {
    const { data, error } = await supabase.from('payment_types').select('*').order('created_at');
    if (error) return res.status(500).json({ message: error.message });
    res.json(data || []);
  });

  router.post('/api/payment-types', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    if (!user || user.role !== 'Admin') {
    ...
    const newPaymentType = {
      id: 'pay-type-' + uuid(), nama_pembayaran, nominal: parseFloat(nominal) || 0, deskripsi: deskripsi || '',
    ...
    if (error) return res.status(500).json({ message: error.message });
    await logActivity(supabase, user.id, user.username, user.role, 'CREATE_PAYMENT_TYPE', `Membuat jenis pembayaran baru: ${nama_pembayaran} (Tarif: Rp ${parseFloat(nominal).toLocaleString('id-ID')})`, req);
    res.status(201).json(data);
  });

  router.put('/api/payment-types/:id', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    ...
    await logActivity(supabase, user.id, user.username, user.role, 'UPDATE_PAYMENT_TYPE', `Mengubah jenis pembayaran: ${nama_pembayaran} (Tarif Baru: Rp ${parseFloat(nominal).toLocaleString('id-ID')})`, req);
    res.json(data);
  });

  router.delete('/api/payment-types/:id', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    if (!user || user.role !== 'Admin') return res.status(403).json({ message: 'Akses ditolak!' });
    const { id } = req.params;
    ...
    if (error) return res.status(500).json({ message: error.message });
    await logActivity(supabase, user.id, user.username, user.role, 'DELETE_PAYMENT_TYPE', `Menghapus jenis pembayaran: ${pt?.nama_pembayaran} beserta tagihan dan cicilan terkait.`, req);
  ...
  router.get('/api/payments', async (req, res) => {
    ...
    if (search) {
      ...
      list = list.filter((p: any) => p.student && (p.student.nama_lengkap.toLowerCase().includes(q) || p.student.nis.includes(q)));
    }
    res.json(list);
  });

  router.post('/api/payments/generate', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    if (!user || (user.role !== 'Admin' && user.role !== 'Bendahara')) {
      return res.status(403).json({ message: 'Akses ditolak!' });
    }
    ...
    for (const student of activeStudents) {
      const { data: exists } = await supabase.from('payments').select('id')
        .eq('student_id', student.id).eq('payment_type_id', payment_type_id).eq('bulan', bulan).eq('tahun', parseInt(tahun, 10)).single();
      if (exists) { skippedCount++; continue; }
      const { error } = await supabase.from('payments').insert({
        id: 'pm-' + uuid(), student_id: student.id, payment_type_id, bulan, tahun: parseInt(tahun, 10),
        nominal_tagihan: payType.nominal, nominal_bayar: 0, sisa_tagihan: payType.nominal, status: 'Belum Lunas',
        created_by: user.id, created_at: new Date().toISOString(), updated_at: new Date().toISOString()
      });
      if (!error) generatedCount++; else skippedCount++;
    }
    if (generatedCount > 0) {
      await logActivity(supabase, user.id, user.username, user.role, 'GENERATE_BILLING', `Men-generate tagihan: ${payType.nama_pembayaran} Periode: ${bulan} ${tahun} untuk ${generatedCount} siswa aktif. (Dilewati: ${skippedCount})`, req);
    }
    res.json({ message: 'Proses generate selesai!', generatedCount, skippedCount });
  });

  router.post('/api/payments/:id/pay', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    if (!user || (user.role !== 'Admin' && user.role !== 'Bendahara')) {
    ...
    if (payment.status === 'Lunas') return res.status(400).json({ message: 'Transaksi ini sudah lunas sebelumnya!' });
    if (payAmount > payment.sisa_tagihan) {
      return res.status(400).json({ message: `Nominal bayar (Rp ${payAmount.toLocaleString('id-ID')}) melebihi sisa tagihan (Rp ${payment.sisa_tagihan.toLocaleString('id-ID')})!` });
    }
    ...
    const { data: updatedPayment, error: updateError } = await supabase.from('payments')
      .update({ nominal_bayar: nominalBayarBaru, sisa_tagihan: sisaTagihanBaru, status: statusBaru, tanggal_bayar: new Date().toISOString(), metode_pembayaran: metode_pembayaran || 'Tunai', bukti_pembayaran_url: bukti_pembayaran_url || payment.bukti_pembayaran_url, catatan: catatan || payment.catatan, updated_at: new Date().toISOString() })
      .eq('id', id).select().single();
    if (updateError) return res.status(500).json({ message: updateError.message });
    const installmentData = {
      id: 'inst-' + uuid(), payment_id: id, tanggal_bayar: new Date().toISOString(), nominal_bayar: payAmount,
      metode_pembayaran: metode_pembayaran || 'Tunai', bukti_pembayaran_url: bukti_pembayaran_url || '', catatan: catatan || '',
    ...
    await logActivity(supabase, user.id, user.username, user.role, 'PAYMENT_INSTALLMENT',
      `Menerima pembayaran Rp ${payAmount.toLocaleString('id-ID')} untuk ${payType?.nama_pembayaran} (${payment.bulan} ${payment.tahun}) siswa ${student?.nama_lengkap}. Status: ${statusBaru}, Sisa: Rp ${sisaTagihanBaru.toLocaleString('id-ID')}`, req);
    res.json({ message: 'Pembayaran berhasil dimasukkan!', payment: updatedPayment, installment });
  });

  router.get('/api/payments/:id/installments', async (req, res) => {
    const { id } = req.params;
  ...
  router.post('/api/payments/:id/upload-bukti', async (req, res) => {
    const { id } = req.params;
  ...
  router.get('/api/dashboard/charts', async (req, res) => {
    ...
    const monthlyRevenue = indonesianMonths.map((m, index) => {
      const rev = installments.filter((inst: any) => new Date(inst.tanggal_bayar).getMonth() === index).reduce((acc: number, inst: any) => acc + inst.nominal_bayar, 0);
      return { name: m.substring(0, 3), nominal: rev };
    });
    const paymentStatusDist = [
      { name: 'Lunas', value: payments.filter((p: any) => p.status === 'Lunas').length, color: '#10B981' },
      { name: 'Cicilan', value: payments.filter((p: any) => p.status === 'Cicilan').length, color: '#F59E0B' },
      { name: 'Belum Lunas', value: payments.filter((p: any) => p.status === 'Belum Lunas').length, color: '#EF4444' }
    ];
    const revenueByClass = ['X', 'XI', 'XII'].map(c => {
      const classStdIds = students.filter((s: any) => s.kelas === c).map((s: any) => s.id);
      const rev = payments.filter((p: any) => classStdIds.includes(p.student_id)).reduce((acc: number, p: any) => acc + p.nominal_bayar, 0);
      const due = payments.filter((p: any) => classStdIds.includes(p.student_id)).reduce((acc: number, p: any) => acc + p.sisa_tagihan, 0);
  ...
  // REPORTS
  // ==========================================
  router.get('/api/reports/rekap-bulanan', async (req, res) => {
    ...
    const persentasePembayaran = totalTagihanKotor > 0 ? parseFloat(((totalPembayaranMasuk / totalTagihanKotor) * 100).toFixed(2)) : 0;
    res.json({ summary: { totalSiswaTertagih, totalTagihanKotor, totalPembayaranMasuk, totalTunggakan, totalSiswaLunas, totalSiswaCicilan, totalSiswaBelumBayar, persentasePembayaran }, details: filteredPayments });
  });

  router.get('/api/reports/tunggakan', async (req, res) => {
  ...
  router.get('/api/system/backup', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    if (!user || user.role !== 'Admin') {
    ...
    const backup = { users: users.data || [], students: students.data || [], payment_types: payment_types.data || [], payments: payments.data || [], installments: installments.data || [], activity_logs: activity_logs.data || [] };
    await logActivity(supabase, user.id, user.username, user.role, 'BACKUP_DB', 'Mengekspor backup basis data sistem.', req);
  ...
  router.post('/api/system/restore', async (req, res) => {
    const user = await getAuthenticatedUser(req);
    if (!user || user.role !== 'Admin') {
